...
Port map ( a=>acc_viejo , b=>op , carryIn=>carryIn_d , carryOut=>carrySuma , sat=>sat_d , restar=>'0' , ov=>ovSuma , res=>suma );
restador: entity work.sumadorRestador
Generic map (N=>DATA_BITS)
Port map ( a=>acc_viejo , b=>op , carryIn=>carryIn_d , carryOut=>carryResta , sat=>sat_d , restar=>'1' , ov=>ovResta , res=>resta );
------------------------------------------------------
...
carryIn_d <=op(0) when (code = "0111") else carryIn_q;  -- El reset lo generan los FFD
...
carryBorrow_d <=carrySuma when (code = "0011") else carryResta when (code = "0100") else
...
registro_carryIn: entity work.FFD
Port map ( clk=>clk , rst=>rst , ena=>ena , d=>carryIn_d , q=>carryIn_q );
------------------------------------------------------
...
-- Rotar en vez de shiftear
-- std_logic_vector(rotate_right(unsigned(carry & acc_viejo),to_integer(signed(op)))) ??